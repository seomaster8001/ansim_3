      ...
      <section className="py-16 md:py-24 px-6 md:px-20 bg-secondary border-b border-border/50">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-body-lg text-body max-w-2xl mb-10 leading-relaxed" style={{ wordBreak: 'keep-all' }}>
            벳맥스는 안심고고의 5단계 검증 프로세스의 모든 항목을 충족하여 Gold Tier로 분류되었습니다.
          </p>
          <div className="space-y-4">
            {[
              { title: '운영 안정성', desc: '5년 이상 안정적 운영 + 0건 누적 제보', status: 'pass' },
              { title: '자본금 검증', desc: '보증금 5억 원 이상 + 운영사 공시 자료 확인', status: 'pass' },
              { title: '입출금 속도', desc: '평균 환전 처리 5분 이내 + 지연 사례 0건', status: 'pass' },
              { title: '고객 응대', desc: '24시간 고객센터 운영 + 응대 만족도 95% 이상', status: 'pass' },
              { title: '보증금 예치', desc: '독립 은행 예치 확인 + 월 1회 재검증', status: 'pass' },
            ].map((item, i) => (
              <div key={i} className="flex items-start gap-4 p-6 rounded-xl bg-background border-2 border-green-200">
